...
const videos: VideoProps[] = [
  { videoId: "7hqffNLydc8", title: "Kominfo 2D Animation Bootcamp - Fajar Abdurrohman" },
...
const AnimationDesign = () => {
  return (
    <section id="animation" className="mb-8 pt-2">
      ...
      <div className="h-[50rem] lg:h-[30rem] shadow-brutalism w-auto rounded-3xl p-2 lg:p-10 flex flex-col lg:flex-row gap-16 items-center justify-center bg-[url('https://ik.imagekit.io/fajarblog/porto//gradient-bg.jpg')] bg-cover"></div>
        {videos.map((video) => (
          <YouTubeEmbed key={video.videoId} videoId={video.videoId} title={video.title} />
        ))}
      </div>
    </section>
  );
