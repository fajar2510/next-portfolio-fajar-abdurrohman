...
const educationData: EducationData[] = [
    {
        ...
        description: 'Build some project and assignment for Web and Android Applications, System Analyst, IT Hardware, Networking, Troubleshooting, and Business. Extra Join game development community and build some games.',
        organizations: [
            'Bidikmisi Scholarship Student Community',
            'Web Game Development Community'
        ],
        publications: [
            { title: 'Interactive Geographic Information System: Visual Mapping, predict Training fund allocation.', link: '#' }
        ],
        // certificates: [
                ...
                {educationData.map((edu, index) => (
                    <li key={index} className="ml-4">
                        ...
                        <time className="mb-1 text-md font-normal leading-none text-slate-700 dark:text-slate-300">
                            {edu.period} <span className='font-bold text-slate-700 italic'>({edu.graduated})</span>
                        </time>

                        <div className='flex flex-wrap items-center justify-start py-2 gap-3'>
                            <div className="overflow-hidden rounded-full">
                                <Image src={edu.logoSrc} width={60} height={60} alt="logo unesa" />
                            </div>
                            <div className='flex flex-col'>
                                <h3 className="text-lg lg:text-xl font-bold text-slate-800 dark:text-white">{edu.university}</h3>
                        ...
                        <p className="text-base font-normal text-slate-700 dark:text-slate-400">{edu.description}</p>
                        <ol>
                            {edu.organizations.map((org, idx) => (
                                <li key={idx} className="text-base font-normal text-slate-700 dark:text-slate-400 inline-block mr-2">
                                    ▫️ <strong>Organization</strong> {org}
                                </li>
                            ))}
                            {edu.publications.map((pub, idx) => (
                                <li key={idx} className="text-base font-normal text-slate-700 dark:text-slate-400">
                                    <a href={pub.link} target='_blank'>▫️ <strong className='underline hover:no-underline hover:text-amber-500 transition-all duration-300 ease-in-out'>E-Journal Publication</strong></a> . {pub.title}
                                </li>
                            ))}
                        </ol>
